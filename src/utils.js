...
const showMealList = () => {
  ...
  item.idMeal
    ...
    .join('')}`;
      ...
      document.querySelector('#counterItem').innerHTML = `Dishes (${countItems(
        res.meals,
      )})`;
        ...
        getComments(id).then((commentsList) => {
          getMealDetalis(id).then((res) => {
            ...
            <div class="d-flex flex-column">
              <h3 class="modal-category">Category: <span class="fw-light">${
  res.strCategory
}</span></h3>
              <h3 class="modal-category">Country: <span class="fw-light">${
  res.strArea
}</span></h3>
              <h3 class="modal-category">Tags: <span class="fw-light">${
  res.strTags
}</span></h3>
            </div>
            ...
            <h5>Add a comment</h5>
            <form id="comments-form" class="d-flex flex-column form form-group">
              <label for="name" hidden>Enter name</label>
              <input type="text" required placeholder="Your Name" id="name" name="name" class="w-50 mb-2 form-control">
              <label for="message" hidden>Enter comment</label>
              <textarea class="form-control" required placeholder="Your Insights" id="message" name="message"></textarea>
            </form>
            `;
            document
